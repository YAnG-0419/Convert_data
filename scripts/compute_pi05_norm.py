...
def main():
    ...
    parser.add_argument("roots", type=Path, nargs="+", help="One or more compatible LeRobot v2.1 datasets")
    parser.add_argument("--repo-id", help="Logical repo id for combined stats; required with multiple datasets")
    ...
    if len(set(roots)) != len(roots):
        parser.error("dataset roots must be unique")
    manifests = [json.loads((root / "conversion/manifest.json").read_text()) for root in roots]
    if len(roots) > 1 and not args.repo_id:
        parser.error("--repo-id is required when combining multiple datasets")
    repo_id = args.repo_id or manifests[0]["config"]["repo_id"]
    ...
    with ExitStack() as locks:
        # Lock in sorted path order to avoid deadlocks and refuse changing inputs.
        for root in sorted(roots):
            lock = locks.enter_context((root / "conversion/lock").open("r"))
        ...
        for root, manifest in zip(roots, manifests, strict=True):
            ...
            datasets.append((root, manifest, info_bytes, info, episodes))
        reference = datasets[0][3]
        for root, _, _, info, _ in datasets[1:]:
            for key in ("codebase_version", "robot_type", "fps", "features"):
                if info.get(key) != reference.get(key):
                    raise ValueError(f"Incompatible {key}: {root}")
        ...
        total_episodes = sum(len(item[4]) for item in datasets)
        completed_episodes = 0
        for dataset_index, (root, _, _, info, episodes) in enumerate(datasets, 1):
            for ep in episodes:
                path = root / info["data_path"].format(episode_chunk=ep["episode_index"] // info["chunks_size"],
                ...
                print(f"stats dataset {dataset_index}/{len(datasets)}, "
                      f"episode {ep['episode_index'] + 1}/{len(episodes)} "
                      f"(combined {completed_episodes}/{total_episodes})", flush=True)
        ...
        provenance = {"datasets": [
                          {"dataset": str(root), "repo_id": manifest["config"]["repo_id"],
                           "dataset_info_sha256": hashlib.sha256(info_bytes).hexdigest(),
                           "frames": info["total_frames"], "episodes": len(episodes)}
                          for root, manifest, info_bytes, info, episodes in datasets],
                      "repo_id": repo_id,
                      "frames": sum(item[3]["total_frames"] for item in datasets),
                      "episodes": total_episodes, "action_horizon": horizon,
